...
The record is one synchronous `writeAtomicDurableSync` on the coordinator thread, from the same finalizer that then withdraws the discovery record. An earlier draft delegated it to a separate helper executable to bound a filesystem stall. That separation does not deliver the property it was introduced for: the next statement in the finalizer is `removeBackendInfoIfOwner` (`src/infra/backend-discovery.ts`), a synchronous read and unlink on the same directory, the same device, and the same thread — not optional, because it _is_ the withdrawal. One shared journal that blocks `fdatasync` blocks a metadata unlink, so the guarded call would have been followed one line later by an unguarded one. Dropping durability does not help either: the atomic write still opens and renames, both journal-bound. What the helper costs against that nothing is a fifth released executable through `scripts/build-server.mjs`, and `strictBundleManifestSchema` (`src/infra/bundle-manifest.ts`) is `.strict()` over a closed set of hash fields, so adding one makes an older build's reader reject the manifest — §10 violated by the fix. The kill mitigation is also false at one of two sync points: `writeAtomicDurableSyncNode` (`src/runtime/real.ts`) is `fdatasync` → `close` → `rename` → directory sync, so a helper killed during the directory sync has already published, while a helper blocked in uninterruptible kernel wait does not die on `SIGKILL` at all — it is reparented, and on device recovery it renames a pre-stall snapshot over whatever a _successor_ coordinator has since written to the same replace-by-instance file. The requirement the record actually carries is ordering — say what you left before withdrawing the address — not survival of a power cut, across which every obligation the record names has been discharged by the same cut. Ordering is free inline. The unbounded withdrawal that remains is recorded in [`docs/todo/discovery-withdrawal-is-unbounded-on-the-exit-path.md`](todo/discovery-withdrawal-is-unbounded-on-the-exit-path.md).
